...
{
    $raw = getenv('HOTEL_STORAGE_MODE');
    $mode = $raw === false ? '' : strtolower(trim((string)$raw));

    if ($mode === 'demo' || $mode === 'request') {
        return 'volatile';
    }

    if (in_array($mode, ['mysql', 'session', 'volatile'], true)) {
        return $mode;
    }
    ...
    if (envBool('HOTEL_DEMO_SESSION_ONLY', false)) {
        return 'session';
    }
...
{
    $mode = getStorageMode();
    if (in_array($mode, ['session', 'volatile'], true)) {
        return true;
    }
...
function getStorageModeNotice(): string
{
    $mode = getStorageMode();

    if ($mode === 'mysql') {
        return 'Persistent MySQL storage mode is active.';
    }

    if ($mode === 'session') {
        return 'Temporary session storage mode is active. Data resets when the browser session expires.';
    }

    return 'Temporary volatile mode is active. Reservations are intentionally short-lived for demo use.';
}

function getStorageDebugSnapshot(): array
{
    return [
        'storage_mode' => getStorageMode(),
        'session_storage' => isSessionStorageMode(),
        'volatile_mode' => getStorageMode() === 'volatile',
        'database_required' => getStorageMode() === 'mysql',
